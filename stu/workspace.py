...
from .constants import LoopPhase, LoopStatus, ProjectScope
from .models import LoopState, Project, WorkspaceManifest
...
def _initial_loop_state(project_id: str) -> LoopState:
    return LoopState(
        loop_id="bootstrap",
...
def _ensure_valid_loop_state(loop_state_file: Path, project_id: str) -> None:
    if loop_state_file.exists() and loop_state_file.is_dir():
        raise ValueError(f"Loop state path is a directory: {loop_state_file}")

    if not loop_state_file.exists():
        loop_state_file.write_text(
            _initial_loop_state(project_id).model_dump_json(indent=2),
            encoding="utf-8",
        )
    ...
    try:
        ...
        backup = loop_state_file.with_name(loop_state_file.name + ".invalid.bak")

        try:
            loop_state_file.replace(backup)
            logger.warning(
                f"Existing loop state was invalid for the current schema. "
                f"Backed up to {backup}. Original error: {exc}"
            )
        except Exception:
            loop_state_file.unlink(missing_ok=True)
            logger.warning(
                f"Existing loop state was invalid and could not be backed up. "
                f"Removed file. Original error: {exc}"
            )
...
def get_project_paths(root: Path, project_id: str, config: AppConfig) -> "ProjectPaths":
    ...
    proj_root = _resolve_inside_root(
        root,
        f"{config.workspace.projects_dir}/{project_id}",
        fail,
    )
...
def bootstrap_workspace(config: AppConfig) -> WorkspaceManifest:
    ...
    paths = {
        "projects": config.workspace.projects_dir,
        "models": config.workspace.models_dir,
        "logs": config.workspace.logs_dir,
        "runtime": config.workspace.runtime_dir,
        "tmp": config.workspace.tmp_dir,
        "artifacts": config.workspace.artifacts_dir,
    ...
    for p in [
        ...
        default_paths.vectors,
        default_paths.vector_store,
    ]:
        _ensure_directory(p)

    if not default_paths.metadata_file.exists():
        proj = Project(
            id=config.app.default_project_id,
            name=config.app.default_project_id,
            scope=ProjectScope.PRIVATE,
        )
        default_paths.metadata_file.write_text(
            proj.model_dump_json(indent=2),
            encoding="utf-8",
        )

    loop_state_file = _resolve_inside_root(
        resolved["runtime"],
        config.execution.loop_state_filename,
        fail,
    )

    _ensure_valid_loop_state(loop_state_file, config.app.default_project_id)

    manifest = WorkspaceManifest(
        root=root,
        projects=resolved["projects"],
        default_project=default_paths.root,
        memory=default_paths.memory,
        memory_l2=default_paths.l2,
        archive=default_paths.archive,
        vectors=default_paths.vectors,
        vector_store=default_paths.vector_store,
